...
export default {
    setup() {
        const dashbaordData = ref(0)
        const dashbaord = async () => {
            try {
                const response = await fetch("/dashbaord")
                const dataResponse = await response.json()
                dashbaordData.value = dataResponse
            } catch (error) {
                console.error("Error fetching dashbaord:", error)
            }
        }
        return {
            dashbaord,
